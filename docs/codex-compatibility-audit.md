...
| Direct Codex chat reasoning | `POST /v1/chat/completions` | Code-supported via provider options, needs separate verification | Codex reasoning remains represented as `provider_options.codex.reasoning`, not as a top-level Chat Completions field. Plan 278 did not smoke direct Chat reasoning. | Direct Codex Chat reasoning should stay in the explicit provider namespace; broad switch-gate readiness still depends on live smokes and tool-family parity. | Add focused provider-option reasoning smoke separately. |
